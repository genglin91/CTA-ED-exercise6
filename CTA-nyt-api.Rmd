...
```{r}
read_______("apikey.env")
load_dot_env(file = "apikey.env")
...
class(json) 
str(json) # list with 3 elements
json$status # This should be "OK"
str(json$response) # the actual data/response in the json object parsed with httr
length(json$response$docs) # The returned documents (capped at 10)
json$response$hits # The meta data of the request (hits returns the total number of articles)
```
...
nyt_count <- function(q, date1, date2) {
  ...
  return(json$response$docs$total)
}
...
```{r}
ggplot(data.frame(year = 2015:2020, count = counts_inequality), aes(x = year, y = count)) +
...
nyt_dates_count <- function(q, init, end, by){
  ...
  df <- data.frame(date = as.Date(dates[-length(dates)], format = "%Y%m%d"),
                    count = counts)
  return(df)
}
```
...
df <- json$response$docs %>% as.data.frame()
df
```
